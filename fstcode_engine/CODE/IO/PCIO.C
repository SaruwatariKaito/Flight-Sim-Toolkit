...
volatile long total_time = 0L;
static volatile long chained_time_ints = 0L;
volatile int rs232_total = 0;

unsigned _stack = 0x10000;

static int processor_speed = 500;
...
/* Estimates errors caused by other interrupts */
static int c_gameport(int *values, int timeout)
{
	static int call_count = 0;
	static int last_v[4], failed[4];
	int i, d, bits, last_bits, buttons;
	int start_time, start_rs232;
	int v[4];
	int times[4], rs232[4];
...
/*	_enable();*/
	for (i = 0; i < 4; i++) {
		if (call_count < 2) {
			...
			if ((times[i] != start_time) || (rs232[i] != start_rs232)) {
				failed[i]++;
				if (failed[i] > 1)
					values[i] = (v[i] + last_v[i]) / 2;
				else
					values[i] = last_v[i];
			} else {
				failed[i] = 0;
...
/*	for (i = 0; i < 2; i++) {
		v[i] += ((times[i] - start_time) * 3000) / processor_speed;
		v[i] += ((chains[i] - start_chain) * 40000) / processor_speed;
		values[i] = v[i];
	}*/
	values[2] = 0;
	values[3] = 0;
...
static void find_processor_speed(void)
{
	int n, v, t1;
