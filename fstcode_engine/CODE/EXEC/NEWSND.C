...
static Sample *engine_sample = NULL;
...
static int jet_sound_playing = FALSE;
...
static void jet_noise(int rpm)
{
    ...
    if (engine_noise_on && engine_on && noise_on) {
		if (sound_driver_on) {
			if (rpm != last_rpm) {
				vol = (aircraft_rpm * 255) / 100;
				set_sample_volume(engine_sample, samples[ENGINE], vol);
				freq = (aircraft_rpm * 255) / 100;
				set_sample_frequency(engine_sample, samples[ENGINE], freq);
				if (!jet_sound_playing) {
					play_sample(engine_sample, TRUE);
					jet_sound_playing = TRUE;
				}
				last_rpm = rpm;
				jet_on = TRUE;
			}
...
{
	if (samples[effect])
		play_sample(samples[effect], FALSE);
}
...
void setup_sound(void)
{
	...
	if (!disable_sounds) {
		if (setup_sblast(0x40000)) {

			samples[CANNON] 	= load_sample("cannon.wav");
			samples[FIRE]   	= load_sample("fire.wav");
			samples[GEAR]   	= load_sample("gear.wav");
			samples[EXPLO1] 	= load_sample("explo1.wav");
			samples[EXPLO2] 	= load_sample("explo2.wav");
			samples[EXPLO3] 	= load_sample("explo3.wav");
			samples[LAND]   	= load_sample("land.wav");
			samples[MAW]    	= load_sample("maw.wav");
			samples[PING]   	= load_sample("ping.wav");
			samples[CLUNK]  	= load_sample("clunk.wav");
			samples[SWLOCK] 	= load_sample("swlock.wav");
			samples[BULLET] 	= load_sample("bullet.wav");
			samples[EJECT]  	= load_sample("eject.wav");
			samples[FLARE]  	= load_sample("flare.wav");
			samples[CHAFF]  	= load_sample("chaff.wav");
			samples[KLAX]   	= load_sample("klax.wav");
			samples[SWITCH] 	= load_sample("switch.wav");
			samples[GLIDE]  	= load_sample("glide.wav");
			samples[LOWWARN] 	= load_sample("lowwarn.wav");
			samples[STALL]		= load_sample("stall.wav");

			samples[ENGINE] 	= load_sample("jet.wav");
			engine_sample		= copy_sample(samples[ENGINE]);

			samples[START] 		= load_sample("start.wav");
			samples[DIE] 		= load_sample("die.wav");
