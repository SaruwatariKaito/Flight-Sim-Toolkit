...
// Perspective correct textured hline
void c_pctex_hline(int x1, int x2, float z1, float z2, int y, float btx1, float btx2, float bty1, float bty2, int tw, uchar *tex)
{
	int x, dx, col, pixels;
	int u, v;
	float tx_grad, ty_grad, tz_grad, dxr;
	uchar *p, line_buffer[MAX_SCREEN_X];
	...
	if (x1 < x2) {
		...
		if (dx > 0) {
			dxr = 1.0f / ITOF(dx);
			tx_grad = (btx2 - btx1) * dxr;
			ty_grad = (bty2 - bty1) * dxr;
			tz_grad = (z2 - z1) * dxr;
		} else {
			tx_grad = btx2 - btx1;
			ty_grad = bty2 - bty1;
		...
		if (dx > 0) {
			dxr = 1.0f / ITOF(dx);
			tx_grad = (btx1 - btx2) * dxr;
			ty_grad = (bty1 - bty2) * dxr;
			tz_grad = (z1 - z2) * dxr;
		} else {
			tx_grad = btx1 - btx2;
			ty_grad = bty1 - bty2;
