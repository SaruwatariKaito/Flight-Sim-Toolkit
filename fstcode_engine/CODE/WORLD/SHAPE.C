 ...
 * GLOBAL find_size
 *
 * Set min_size and max_size to bounding box of shape sd.
 * Looks at most detailed level of shape.
 * 
 */
...
/* -----------------------------------------------
 * GLOBAL find_size_all
 *
 * Set min_size and max_size to bounding box of shape sd.
 * Looks at all levels in the shape.
 * 
 */
...
/* -------------------
 * GLOBAL recolour_shape
 * 
 * Re-shade shape polygons based on sun position and
 * rotaion of shape (r).
 * 
 */
 ...
 * GLOBAL get_shape
 * 
 * Check to see if shape is loaded - if so return it,
 * else load shape and put it on shapes list.
 * 
 * If shape not found loads and returns BOX.FSD.
 * 
 */
...
static int load_texture_file(char *texture_file, int base_col)
{
	...
	if (fp != NULL) {
		...
		map = (uchar *)heap_alloc(width * height);
		if (map == NULL)
			stop("No memory for texture map from %s", texture_file);
		grab_texture(map, texture_pic, PCX_info.width, base_col, 0, 0, width, height);
		do {
...
static void set_poly_texture2(sShape *s, sPolygon *poly)
{
	...
	if (strcmp(shape_name, texture_shape_name) != 0) {
		...
		clear_texture_defs();
		found = load_texture_file(texture_file, base_col);
		if (!found) {
			stop("Texture file %s not found", texture_file);
		}
	}
